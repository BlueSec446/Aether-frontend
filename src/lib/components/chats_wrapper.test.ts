...
describe('Chats Controller', () => {
    ...
    describe('loadChats', () => {
        const mockChats: Chat[] = [
            { chat_id: 1, title: 'Alice', is_group: 0, display_name: 'Alice', contact_ids: [{contact_id: 1}], last_message: null },
            { chat_id: 2, title: 'Bob', is_group: 0, display_name: 'Bob', contact_ids: [{contact_id: 1}], last_message: null }
        ];
    ...
    describe('processSyncData', () => {
        const incomingMsg = { id: 100, chat_id: 1, content: "Hello", status: "INCOMING_UNREAD" } as Message;
        ...
        it('updates chatStore when new messages arrive for an inactive chat', () => {
            chatStore.setChats([{ chat_id: 1, is_group: 0, display_name: '', title: '', contact_ids: [{contact_id: 1}], last_message: null }]);
